    ...
    return (
      <main className="flex min-h-screen items-center justify-center bg-slate-950 px-4">
        <div className="w-full max-w-md rounded-xl border border-slate-800 bg-slate-900 p-8 text-center">
          ...
          <h1 className="text-lg font-semibold text-slate-100">Report unavailable</h1>
  ...
  return (
    <main className="min-h-screen bg-slate-950 text-slate-100">
      <div className="mx-auto max-w-3xl px-4 py-12 sm:px-6">
        ...
        <p className="text-xs font-medium uppercase tracking-wide text-slate-500">{report.period}</p>
        <h1 className="mt-1 text-2xl font-bold text-slate-100 sm:text-3xl">{report.title}</h1>

        {/* Headline recoverable */}
        <div className="mt-8 rounded-2xl border border-emerald-500/40 bg-gradient-to-br from-emerald-400/10 to-slate-900 p-8 text-center">
          <div className="text-xs font-medium uppercase tracking-wide text-emerald-300/80">
            Recoverable spend
          </div>
          <div className="mt-2 text-5xl font-black tabular-nums text-emerald-300">
        ...
        <div className="mt-6 grid grid-cols-2 gap-4 sm:grid-cols-4">
          <div className="rounded-xl border border-slate-800 bg-slate-900 px-4 py-3">
            <div className="text-[11px] font-medium uppercase tracking-wide text-slate-500">Total spend</div>
            <div className="mt-1 text-lg font-bold tabular-nums text-slate-100">
              {fmtMoney(report.total_spend_cents)}
            </div>
          </div>
          <div className="rounded-xl border border-slate-800 bg-slate-900 px-4 py-3">
            <div className="text-[11px] font-medium uppercase tracking-wide text-slate-500">Non-prod spend</div>
            <div className="mt-1 text-lg font-bold tabular-nums text-slate-100">
              {fmtMoney(report.nonprod_spend_cents)}
            </div>
          </div>
          <div className="rounded-xl border border-slate-800 bg-slate-900 px-4 py-3">
            <div className="text-[11px] font-medium uppercase tracking-wide text-slate-500">Idle waste</div>
            <div className="mt-1 text-lg font-bold tabular-nums text-red-300">
              {fmtMoney(report.idle_waste_cents)}
            </div>
          </div>
          <div className="rounded-xl border border-slate-800 bg-slate-900 px-4 py-3">
            <div className="text-[11px] font-medium uppercase tracking-wide text-slate-500">Recovered</div>
