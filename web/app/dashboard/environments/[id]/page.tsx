...
function heatColor(value: number, max: number): string {
  if (!max || value <= 0) return 'rgba(63,63,70,0.35)' // slate-700 faint
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <Link href="/dashboard/environments" className="text-sm text-slate-500 hover:text-slate-300">
            ← Environments
          </Link>
          <div className="mt-1 flex items-center gap-3">
            <h1 className="text-2xl font-bold text-slate-100">{env?.name}</h1>
            <Badge tone={envTone(env?.env_kind)}>{env?.env_kind ?? 'unclassified'}</Badge>
            {env?.is_production && <Badge tone="danger">production</Badge>}
          </div>
          {env?.description && <p className="mt-1 text-sm text-slate-500">{env.description}</p>}
        </div>
        <div className="text-right text-xs text-slate-500">
          <div>Timezone: <span className="text-slate-300">{env?.timezone ?? 'UTC'}</span></div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        <Card>
          <CardHeader>
            <h2 className="text-base font-semibold text-slate-100">Timezone, Schedule & Calendar</h2>
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center justify-between">
              <div>
                <h2 className="text-base font-semibold text-slate-100">Idle Heatmap</h2>
                <p className="mt-0.5 text-xs text-slate-500">Hour-of-week idle intensity</p>
              </div>
                ...
                <div className="min-w-[560px]">
                  ...
                  <div className="flex">
                    ...
                    {Array.from({ length: 24 }).map((_, h) => (
                      <div
                        key={h}
                        className="flex-1 pb-1 text-center text-[9px] tabular-nums text-slate-600"
                      >
                        {h % 3 === 0 ? h : ''}
                      </div>
                    ))}
                  ...
                  {heatmap.map((row, d) => (
                    <div key={d} className="flex items-center">
                      <div className="w-10 shrink-0 pr-1 text-right text-[10px] text-slate-500">
                        {DAYS[d] ?? `D${d}`}
                      </div>
                      {row.map((v, h) => (
                        <div
